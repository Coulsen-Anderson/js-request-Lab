...
const repeatMyParam = () => {
    //YOUR CODE HERE
    axios.get(`${baseUrl}/repeat/{'I love coding so much omg'}`)
    .then((res) =>{
        console.log(res.data)
        let characterArr = res.data
        // for(let i = 0; i < characterArr.length; i++){
        //     createCharacterCard(characterArr[i])
        // }
        repeatText.textContent = characterArr
...
// YOUR CODE HERE
const myQuery = () => {
    axios.get(`${baseUrl}/repeat/{'Coulsen is the Best'}`)
   .then((res) =>{
    console.log(res.data)
    let characterArr = res.data
    // for(let i = 0; i < characterArr.length; i++){
